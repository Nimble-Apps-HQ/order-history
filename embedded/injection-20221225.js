(() => {
  let isLocal = ['localhost', '127.0.0.1'].includes(location.hostname);
  const isAppBlock = document.querySelector('script[data-nim-order-history]').getAttribute('data-nim-order-history-app-block') != null;

  if (isAppBlock) {
    isLocal = false;
  }
  
  const apiLink = isLocal ? 'http://localhost:59424' : 'https://api-server-a2d7o3v32a-uc.a.run.app';
  ...
  xmlHttpRequest.onreadystatechange = function() {
    if (xmlHttpRequest.readyState === XMLHttpRequest.DONE && xmlHttpRequest.status === 200) {
      ...
      if (responseModel && responseModel.status === 'SUCCESS' && responseModel.data) {
        ...
        const injectionElement = document.querySelector('script[data-nim-order-history]');
        const injectionContainer = injectionElement.parentElement || document.body;

        const mountingPoint = document.createElement('div');
        mountingPoint.setAttribute('id', mountingPointId);
        mountingPoint.dataset.appId = appId;
        injectionContainer.appendChild(mountingPoint);
        ...
        console.log('No-login Order History Search experiencing technical issue. Please contact developer for support.');
      }
    }
  }
